    ...
    private suspend fun handleClientConnection(socket: Socket, clientIp: String) {
        try {
            ...
            if (uri == "/" || uri == "") {
                ...
                val curScreenBlack = prefs.getBoolean("screen_black_mode", false)
                ...
                val curFps = getPrefInt("camera_fps", 30)
                val fpsMin = prefs.getInt("camera_fps_min", 1)
                val fpsMax = prefs.getInt("camera_fps_max", 60)
                ...
                val htmlResponse = htmlTemplate
                    ...
                    .replace("{{SCREEN_ACTIVE}}", if (curScreenBlack) "active" else "")
                    .replace("{{SCREEN_LABEL}}", if (curScreenBlack) "Black" else "Live")
                    ...
                    .replace("{{CUR_FPS}}", curFps.toString())
                    .replace("{{FPS_MIN}}", fpsMin.toString())
                    .replace("{{FPS_MAX}}", fpsMax.toString())
